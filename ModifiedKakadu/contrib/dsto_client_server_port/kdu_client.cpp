/*****************************************************************************/
// File: kdu_client.cpp [scope = CONTRIB/DSTO-CLIENT-SERVER-PORT]
// Version: Kakadu, V6.1
...
#define KDU_WARNING_DEV(_name,_id) KDU_WARNING(_name,_id)
 // Use the above version for warnings which are of interest only to developers
            ...
            else if ((qp[0] == 'T') && (qp[1] == '='))
              {
                ...
                if ((sscanf(qp+2,"%f",&val) != 1) || (val <= 0.0F))
                  { kdu_error e; e << "Malformed time (T) field in "
                    "\"simulated_requests.src\" line:\n\n" << line; }
                req->duration = (time_t)(0.5F+SYS_CLOCKS_PER_SEC*val);
                if (req->duration == 0)
                  req->duration = 1;
              }
            else
    ...
    kd_request_simulator::can_advance()
  {
    ...
    time_t now = sysClock() - base_time;
    if (now >= advance_gate)
      return 0;
    return (kdu_uint32) ((1000*(advance_gate-now)) / SYS_CLOCKS_PER_SEC);
  }
  ...
  void
    kd_request_simulator::allow_advance()
  {
    advance_gate = sysClock() - base_time;
    if (advance_gate == 0)
      advance_gate = 1;
  }
                                            ...
                                            int extracted_bytes)
  {
    ...
    time_t now = sysClock() - base_time;
    float fnow = ((float) now) / SYS_CLOCKS_PER_SEC;
...
extern "C" {
  static void* thread_start_proc(void *param)
  {
...
#define ONE_SECOND SYS_CLOCKS_PER_SEC
...
kd_client::kd_client(kdu_client *owner)
{
  ...
  wsa_event = sysCreateEvent(FALSE);
  h_thread = 0;
  ...
  kd_client::start()
{
  ...
  h_thread = sysStartThread(thread_start_proc, this, TRUE);
}
  ...
  kd_client::network_closing()
{
  ...
  sysResetEvent(wsa_event); // Just in case
  ...
  if (!thread_closed)
    {
      ...
      sysSetEvent(wsa_event); // Try to wake up thread, asking it to die, then wait
      owner->acquire_lock();
      if (pending_name_resolution)
        sysKillThread(h_thread);
      owner->release_lock();
    }
  ...
  if (!immediate_address_resolved)
    {
      ...
      immediate_address.sin_addr.s_addr =
  ...
  if (request_channel->is_open())
    {
      ...
      if ((existing_address.sin_addr.s_addr ==
           immediate_address.sin_addr.s_addr) &&
  ...
  while (!(succeeded || attempted_channel_open))
    {
      try {
        ...
        { // Catch exceptions in channel opening or channel writing
          if (!thread_closure_requested)
            {
              // avoid abort if server not available
  ...
  kd_client::network_manager()
{
  ...
  while (!thread_closure_requested)
    {
      kdu_byte *raw = NULL;
      int   wait_milliseconds = 5000; // Occasionally network events don't fire
        ...
        { // Pipeline requests only if connection is persistent and involves
          ...
          if ((current_time >= request_delivery_gate) ||
            ...
            { // Send request and wait > 1 seconds before considering another
              ...
              // requests.
              deliver_new_request();
              request_delivery_gate = current_time + SYS_CLOCKS_PER_SEC;
            }
          else
      ...
      if ((!in_http_body) && (request_channel != NULL) &&
        ...
        { // Wait for event to be set.
          DWORD wait_result =
            sysWaitForEvent(wsa_event, wait_milliseconds);
          if ((wait_result != SYS_WAIT_EVENT_0) &&
              (wait_result != SYS_WAIT_TIMEOUT))
            return;
        }
    }
}
  ...
  kdu_client::close()
{
  ...
  idle_state = false;
  status = "<not in use>";
  // IEL: The first thing the open does is a "just in case" close so if a
  //      notifier was installed prior to open it would be discarded!
  //      Hence the following removed:
  //notifier = NULL;
  notifier = NULL;
  received_bytes = 0;
  return true;
