/*****************************************************************************/
// File: connection.cpp [scope = CONTRIB/DSTO-CLIENT-SERVER-PORT]
// Version: Kakadu, V6.0
...
extern "C" {
  static void *kd_connection_thread_start_proc(void *param)
  {
...
void
  kd_connection_thread::start()
{
  h_thread = sysStartThread(kd_connection_thread_start_proc,this,FALSE);
}
  ...
  while (1)
    {
      ...
      if (host->resolution_counter == 1)
        { // Attempt to resolve host name
          ...
          host->address.sin_addr.s_addr = inet_addr(host->hostname);
          if (host->address.sin_addr.s_addr == INADDR_NONE)
