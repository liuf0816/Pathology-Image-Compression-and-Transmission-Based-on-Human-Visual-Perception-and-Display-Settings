/*****************************************************************************/
// File: kdu_server.cpp [scope = CONTRIB/DSTO-CLIENT-SERVER-PORT]
// Version: Kakadu, V6.0
     ...
     return string, if non-NULL, is a password. */
{
  ...
  initial_timeout_milliseconds = 500;
  completion_timeout_milliseconds = 2000;
...
/* IEL: Actually just uses 4 byte "hostid" */
{
  long *addrPtr;

  addrPtr = (long *)address;

  *addrPtr = gethostid();

  address[4] = 'I'; /* pad to full 6 bytes */
  address[5] = 'L';
}
                  ...
                  const char *cache_directory)
{
  ...
  if (stat(target.filename, &statBuff) != 0)
    {
      explanation << "404 Image does not exist on server";
      return false;
    }
  kdu_byte last_modify_buf[4];
  for (int n=0; n < 4; n++)
    {
      last_modify_buf[n] = (kdu_byte)(statBuff.st_mtime >>(24-n*8));
    }
    ...
    { // Read the target-ID
      ...
      for (int i=0; (i < 32) && cache_exists; i++)
        if (!(((target_id[i] >= '0') && (target_id[i] <= '9')) ||
              ...
              ((target_id[i] >= 'A') && (target_id[i] <= 'F'))))
          cache_exists = false;
      char test_modify_buf[4];
      if ((fread(test_modify_buf,1,4,cache_fp) != 4) ||
          (memcmp(test_modify_buf,last_modify_buf,4) != 0))
        cache_exists = false;
      if (!cache_exists)
            ...
            { // Try to create the cache file
              ...
              if ((cache_fp != NULL) &&
                  ((fwrite(target_id,1,32,cache_fp) != 32) ||
                   (fwrite(last_modify_buf,1,4,cache_fp) != 4)))
  ...
  kd_source::generate_target_id()
{
  ...
  if (stat(target.filename, &statBuff) != 0) return false;

  union {
      kdu_byte id_data[16];
      kdu_uint32 id_data32[4];
    };
  id_data32[0] = statBuff.st_mtime;
  id_data32[1] = statBuff.st_ctime;
  id_data32[2] = id_data32[3] = 0;

  int i;
  char full_pathname[PATH_MAX];

  full_pathname[0]=full_pathname[PATH_MAX - 1]='\0';
  if (!realpath(target.filename, full_pathname))
...
extern "C" {
  static void *kd_source_thread_start_proc(void *param)
  {
...
{
  assert(h_thread == 0);
  h_thread = sysStartThread(kd_source_thread_start_proc, this, FALSE);
      ...
      if (!((waiting_for_primary_channel && (primary_channel!=NULL)) ||
        ...
        { // No connections are ready to be installed yet and no data is
          ...
          if (waiting_for_primary_channel || waiting_for_auxiliary_channel)
            ...
            if (sysWaitForEvent(h_event,completion_timeout) != SYS_WAIT_EVENT_0)
              {
                  return; 
              }
            }
...
{
  int flags; // setsockopt flags for disable TCP Nagle algorithm
  // make sure the clock is initialised before use in any thread
  sysClock();
  ...
  kd_delegator delegator;
  HANDLE listen_event = sysCreateEvent(TRUE); // Manual-reset
  ...
  try { // Catch otherwise uncaught exceptions thrown by `error_callback'.
      ...
      /* Call setsockopt so that we can re-bind to the socket without */
      /*  TIME_WAIT problems */
      ...
      if (listen_ip_addr != INADDR_NONE)
        listen_addr.sin_addr.s_addr = listen_ip_addr;
      else
        {
      ...
      while ((sysEventSelect(listen_socket, listen_event, SYS_READ) == 0) &&
             (sysWaitForEvent(listen_event,INFINITE) == SYS_WAIT_EVENT_0) &&
             !source_manager->check_shutdown_requested())
        {
          struct sockaddr  entAddr;
          socklen_t        addrLen = sizeof(struct sockaddr);

          if ((connected_socket =
               accept(listen_socket,&entAddr,&addrLen)) != INVALID_SOCKET)
            {
              flags = 1;
              setsockopt(connected_socket, /* socket affected */
                         IPPROTO_TCP,      /* set option at TCP level */
                         TCP_NODELAY,      /* name of option */
                         (char *) &flags,  /* the cast is historical cruft */
                         sizeof(int));     /* length of option value */
