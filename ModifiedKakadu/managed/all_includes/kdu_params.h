...
class kdu_params {
  /* [BIND: reference]
     [SYNOPSIS]
       ...
       of the `kdu_params' object whose tile and/or component index is equal
       ...
       in the cluster.  It is possible for this to cause some confusion,
       since the `access_relation' member may return a pointer to a different
       `kdu_params' object to the one you expect.  If you then modify any
       of the parameter values, you may end up modifying the wrong object.
       To avoid this difficulty, the `access_relation' function takes a
       special argument to identify whether you wish to modify any parameters
  ...
  public:
    KDU_EXPORT virtual ~kdu_params();
      /* [SYNOPSIS]
           ...
           both negagive), the entire cluster will be destroyed.
           [//]
  ...
  public: // Navigation and identification member functions
    ...
    KDU_EXPORT kdu_params *
      ...
      /* [SYNOPSIS]
           ...
           should be aware that there may not be a unique `kdu_params'
...
class siz_params: public kdu_params {
  /* [BIND: reference]
     ...
     [>>] Macro = `Sprofile';
          ...
          value must currently be an integer in the range 0 to 5.  Profile0
          ...
          Digital Cinema.  Even though these have a higher numerical value
          that Part 2 codestreams, this is only for the purpose of backward
          compatibility with earlier versions of Kakadu.  The 2K and 4K
          digital cinema profiles are actually very restrictive subsets of
          JPEG2000 Part 1, defined by an ammendment to the original standard.
          If the system determines that support for Part 2 features is
          required, the profile will be set automatically to 3.  Otherwise,
          the profile is not adjusted by Kakadu's codestream creation
          machinery.  However, if the profile is found to be insufficient,
          the system will generate a warning at the point where it first
          encounters an inconsistency; this might not occur until near the
          end of the processing in certain rare circumstances.  The
          system does perform some extensive checks for compliance with the
          Digital Cinema profiles when they are used, but only during
          ...
          user to set the `Creslengths' attributes to ensure that compressed
          data sizes match the application-dependent constraints specified
          by the Digital Cinema ammendment.  The default value for the
          `Sprofile' attribute is Profile2.  When setting the
          profile, you may find it convenient to use the macro's
          `Sprofile_PROFILE0', `Sprofile_PROFILE1', `Sprofile_PROFILE2',
          `Sprofile_CINEMA2K', `Sprofile_CINEMA4K' and `Sprofile_PART2'.
     [>>] Macro = `Scap'; Pattern = "B" --
          ...
          are defined in a separate capabilities marker segment.
...
class mcc_params: public kdu_params {
  /* [BIND: reference]
     ...
     [>>] Macro = `Mstage_xforms';
          ...
          and `Kcoeffs' attributes describe the DWT kernel.
