...
class kdu_thread_entity {
  /* [BIND: reference]
     [SYNOPSIS]
       ...
       catching and rethrowing is restricted to integer-valued exception
       codes.
       [//]
  ...
  public: // Members
    ...
    KDU_EXPORT bool
      ...
      /* [SYNOPSIS]
           ...
           thrown, have integer values.
           [//]
           If `throw_group_failure' is false, the function catches integer-valued
           exceptions thrown while processing jobs and automatically invokes the
           `handle_exception' function for you.  Moreover, with
           `throw_group_failure' false, the function neither throws exceptions
           nor returns, unless either a wait condition is satisfied (returns
           true) or destruction is requested (returns false).  This means
           that errors can go undetected, which is not normally desirable.
           It is desirable, however, when the function is executed from within
           the thread entry-point function, since it ensures that the thread
           scheduling algorithm continues to operate across error conditions
           until such time as the entire multi-threaded environment is to
           be destroyed.
           [//]
         ...
         [ARG: throw_group_failure]
           ...
           unexpected failures cause an integer-valued exception to be thrown.
           The internal machinery, however, sets this argument to false in
           certain circumstances.
      */
    virtual void handle_exception(int exc_code);
      /* [SYNOPSIS]
           ...
           throw an integer-valued exception.  In the former case, no cleanup
           is required.  When an exception is caught, however, two
    ...
    void acquire_lock(int lock_id, bool allow_exceptions=true)
      { kd_thread_lock *lock = locks + lock_id;
        ...
        if (allow_exceptions && grouperr->failed) throw grouperr->failure_code;
      ...
      /* [SYNOPSIS]
           ...
           [//]
           Note that this function may throw an integer-valued exception if
           any other thread in the group terminates unexpectedly, invoking
           its `handle_exception' function.  To avoid this (e.g., when
           performing cleanup operations after an exception has already
           been thrown), set the `allow_exceptions' argument to false.
         [ARG: lock_id]
