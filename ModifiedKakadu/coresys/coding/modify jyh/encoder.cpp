...
double *find_p_weight(char *fname, int size_x, int size_y, int pos_x, int pos_y,int level,int num_coeffs,int orient) {
	int num_components;
	int height, width;
	...
	double *p_weight = new double[num_coeffs];
	double area=0;
	int ipos_x = pos_x;
	int ipos_y = pos_y;
	...
	//p_weight = (128 / (1 + exp(50 * p - 25))) / pow(2, 2*level) + 1;
	//其中上下限用于改善整体模糊度，与P相连的两个系数改变平滑度和阈值。
	...
	for (int i = 0; i < num_coeffs; i++) {
		sum = 0;
		area = 0;
		int i_y = floor(i / size_x);
		int i_x = i%size_x;
		int i_corx = 0;
		int i_cory = 0;

		for (int ki = 0; ki < pow(2, level); ki++) {
			for (int kj = 0; kj < pow(2, level); kj++) {
				i_corx = ipos_x + i_x*pow(2, level) + ki;
				i_cory = ipos_y + i_y*pow(2, level) + kj;


				if ((i_cory < height)&(i_corx < width)) {
					sum = sum + (int)buffer[i_cory*width + i_corx];
		...
		p_weight[i]= (1280 / (1 + exp(100 * p -90 ))) + 1;
  ...
  kd_encoder::do_job(kdu_thread_entity *ent, int job_idx)
{
  ...
  for (; blocks_remaining > 0; blocks_remaining--,
       idx.x++, offset+=xfer_size.x)
    {
	  ...
	  //block->p_weight = find_p_weight(p_fname, block->size.x, block->size.y, oh_coords.x, oh_coords.y,block->dwt_level,num_coeffs,block->orient);
