...
kdu_params::~kdu_params()
{
  ...
  if (refs != NULL)
    { // Otherwise, we have already visited the components columnwise
      assert(refs[ref_idx] == this);
      refs[ref_idx] = NULL;
      ...
      for (sref=refs+ref_idx, n=num_tiles; n > 0; n--)
        {
          ...
          if (*sref == this)
            *sref = NULL;
            ...
            {
              if (comp_idx < 0)
                (*sref)->refs = NULL; // So don't delete columns over again
              delete *sref;
            }
        }
    }
  ...
  : kdu_params(SIZ_params,false,false,false)
{
  define_attribute(Sprofile,
          ...
          "value must be an integer in the range 0 to 5, corresponding "
          "to the identifiers `PROFILE0', `PROFILE1', `PROFILE2', "
          "`PART2', `CINEMA2K' and `CINEMA4K'.  PROFILE0 is the most "
          "restrictive profile for Part 1 conforming codestreams.  PROFILE2 "
          "places no restrictions on the code-stream other than those "
          "restrictions defined in ISO/IEC 15444-1 (JPEG2000, Part 1).  A "
          "value of 3 (`PART2') means that the codestream requires support "
          "for one or more features defined in ISO/IEC 15444-2 (JPEG2000, "
          "Part 2).  If the system determines that support for Part 2 "
          "features is  required, the profile will be set automatically to "
          "3.  Otherwise, the profile is not adjusted by Kakadu's codestream "
          "creation machinery.  However, if the profile is found to be "
          "insufficient, the system will generate a warning at the point "
          "where it first encounters an inconsistency; this might not occur "
          "until near the end of the processing in certain rare "
          "circumstances.  The `CINEMA2K' (4) and `CINEMA4K' (5) profiles "
          "were added later via an ammendment to Part 1.  They are in fact "
          "highly restrictive subsets of the Part 1 standard, taylored for "
          "the needs of digital cinema projection equipment.  To be sure "
          "that you generate code-stream compatible with one of the digital "
          "cinema profiles, you should use the \"Creslengths\" attributes "
          "to explicitly constrain the compressed lengths of "
          "resolution/component subsets.  The actual constraints depend "
          "upon the intended frame-rate, as described in the standard.  "
          "Some suggestions are provided via a warning message if "
          "you fail to supply the `Creslengths' attribute yourself.\n"
                                   ...
                                   int tpart_idx)
{
  ...
  else if ((extensions != 0) || (profile > 5) || (profile < 0))
    { KDU_ERROR(e,0x21040502); e <<
        KDU_TXT("Illegal profile index.  \"Sprofile\" must be set "
        "in the range 0 to 5; 3 is required if Part 2 features are to be "
        "used.");
    }
  else if (profile == 2)
    r_siz = 0; // Generic Part-1 codestream
  else if (profile > 3)
    r_siz = (kdu_uint16)(profile-1); // Digital Cinema profiles
  else
    r_siz = (kdu_uint16)(profile+1); // Part-1 Profile-0/Profile-1 codestream
  if (uses_cap)
    r_siz |= 0x4000;
          ...
          else
            {
              ...
              if ((profile < 0) || (profile > 4))
                { KDU_ERROR(e,0x21040503); e <<
                    KDU_TXT("Invalid SIZ marker segment encountered!");
                }
              if (profile == 0)
                profile = 2; // The unrestricted profile (Profile-2) has Rsiz=0
              else if (profile >= 3)
                profile++;
              else
                profile--;
  ...
  siz_params::finalize(bool after_reading)
{
  ...
  int profile;
  if (!get(Sprofile,0,0,profile))
    set(Sprofile,0,0,profile=2);
  if ((profile < 0) || (profile > 5))
    { KDU_ERROR(e,89); e <<
        KDU_TXT("Illegal profile index.  \"Sprofile\" must be in the "
        "range 0 to 5: Profile-0 is the most restrictive original Part-1 "
        ...
        "ammendment to Part-1 of the standard.");
    }
  ...
  // Perform some Digital Cinema checks
  ...
  if (profile != new_profile)
    { KDU_WARNING(w,0x11110810); w <<
        KDU_TXT("Code-stream profile had to be changed from a digital cinema "
                "profile to a PART2 profile, since one or more PART2 "
                "features have been used.");
      return;
    }
  int sval=0;
  for (n=0; n < components; n++)
    if (!(get(Ssampling,n,0,sval) && (sval == 1) &&
          get(Ssampling,n,1,sval) && (sval == 1) &&
          get(Ssigned,n,0,sval) && (sval == 0) &&
          get(Sprecision,n,0,sval) && (sval == 12)))
      break;
  if ((components != 3) || (n != 3))
    { KDU_ERROR(e,0x11110811); e <<
        KDU_TXT("Profile violation detected.  Code-streams marked with a "
                "Digital Cinema profile (CINEMA2K or CINEMA4K) must have "
                "exactly 3 image components, each unsigned with a precision "
                "of 12 bits and all sub-sampling factors equal to 1.");
    }
  if ((origin_x != 0) || (origin_y != 0) || (tile_ox != 0) || (tile_oy != 0))
    { KDU_ERROR(e,0x11110812); e <<
      KDU_TXT("Profile violation detected.  Code-streams marked with a "
              "Digital Cinema profile (CINEMA2K or CINEMA4K) must have "
              "their image and tile partition anchored a the origin.");
    }
  if ((size_x < tile_x) || (size_y < tile_y))
    { KDU_ERROR(e,0x11110813); e <<
      KDU_TXT("Profile violation detected.  Code-streams marked with a "
              "Digital Cinema profile (CINEMA2K or CINEMA4K) must have "
              "only one tile.");
    }
  ...
  if (org != NULL)
    {
      org->set(ORGgen_tlm,0,0,(profile==Sprofile_CINEMA2K)?3:6);
      int tparts = 0;
      if (!org->get(ORGtparts,0,0,tparts))
        org->set(ORGtparts,0,0,tparts=ORGtparts_C);
      if (tparts != ORGtparts_C)
        { KDU_WARNING(w,0x11110815); w <<
          KDU_TXT("Profile violation detected.  Code-streams marked with a "
                  "Digital Cinema profile (CINEMA2K or CINEMA4K) must have "
                  "tile-parts generated according to the \"ORGtparts=C\" "
                  "flavour.  Letting this go for now, since you may be "
                  "intending to reorganize tile-parts yourself later on.");
        }
    }
  ...
  if (!cod_root->get(Clevels,0,0,num_levels))
    cod_root->set(Clevels,0,0,num_levels=6);
  ...
  : kdu_params(MCC_params,true,false,true,false,true)
{
  ...
  define_attribute(Mstage_xforms,
    ...
    "`Ksteps' and `Kcoeffs' attributes describe the DWT kernel.\n"
                                   ...
                                   int tpart_idx)
{
  ...
  if ((comp_idx >= 0) &&
      ((profile == Sprofile_CINEMA2K) || (profile == Sprofile_CINEMA4K)) &&
      (ref != NULL) && !ref->compare(Clevels,0,0,levels))
    { KDU_ERROR(e,0x11110808); e <<
        KDU_TXT("About to write a component-specific COD/COC marker "
                "segment, which identifies a different number of DWT "
                "decomposition levels (Clevels) to that identified in the "
                "main COD marker segmen.  This is not legal for the "
                "Digital Cinema profiles CINEMA2K or CINEMA4K)");
    }
  ...
  cod_params::finalize(bool after_reading)
{
  ...
  if ((!get(Ckernels,0,0,kernels)) && !after_reading)
    set(Ckernels,0,0,kernels=Ckernels_W9X7);
  ...
  if (dcinema)
    {
      get(Clayers,0,0,val);
      if (val != 1)
        { KDU_WARNING(w,0x11110801); w <<
          KDU_TXT("Profile violation detected.  The Digital "
                  "Cinema profiles (CINEMA2K or CINEMA4K), "
      ...
      if ((blk_size.x != 32) || (blk_size.y != 32))
      ...
      if (modes != 0)
        { KDU_ERROR(e,0x11110805); e <<
          KDU_TXT("Profile violation detected.  Requested block coding mode "
                  "is incompatible with the use of a Digital Cinema profile "
                  "(CINEMA2K or CINEMA4K).");
        }
      for (int lev=0; lev <= dwt_levels; lev++)
        {
          ...
          if (precinct_siz != reqd_siz)
            { KDU_ERROR(e,0x11110806); e <<
              KDU_TXT("Profile violation detected.  Requested precinct "
                      ...
                      "Cinema profile (CINEMA2K or CINEMA4K).  All precincts "
                      "need to be of size 256x256 except the lowest "
                      "resolution level which needs 128x128");
            }
        }
      if ((this->comp_idx < 0) && !get(Creslengths,0,0,val))
  ...
  if (profile == Sprofile_CINEMA2K)
    {
      if (dwt_levels > 5)
        { KDU_ERROR(e,0x11110803); e <<
          KDU_TXT("Profile violation detected.  The CINEMA2K profile "
                  "allows at most 5 DWT decomposition levels.  You "
                  "have requested " << val << " levels, though.");
        }
      if (progression_order != Corder_CPRL)
        { KDU_ERROR(e,0x1111081a); e <<
          KDU_TXT("Profile violation detected.  The CINEMA2K profile "
                  "requires a CPRL packet progression order.");
        }
    }
  else if (profile == Sprofile_CINEMA4K)
    {
      if (dwt_levels > 6)
        { KDU_ERROR(e,0x11110804); e <<
          KDU_TXT("Profile violation detected.  The CINEMA4K profile "
                  "allows at most 6 DWT decomposition levels.  You "
                  "have requested " << val << " levels, though.");
        }
    }                  
}
    ...
    { // Ignore any existing step sizes in this case.
      if (get(Qabs_steps,0,0,fval,false,false,false))
        { KDU_WARNING_DEV(w,6); w <<
            ...
            "inheritance path of the relevant tile-component.");
        }
                                   ...
                                   int tpart_idx)
{
  ...
  if (siz != NULL)
    siz->get(Sprofile,0,0,profile);
  if (profile == Sprofile_CINEMA2K)
    { KDU_ERROR(e,0x11110830); e <<
      KDU_TXT("Profile violation detected.  POC marker segments may not "
              "be included in 2K Digital Cinema (CINEMA2K) code-streams.  "
              "You should either remove the `Sprofile' attribute or remove "
              "the `Porder' attribute.");
    }
