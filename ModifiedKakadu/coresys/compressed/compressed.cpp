      ...
      for (r=tc->dwt_levels; r >= 0; r--)
        {
          ...
          res->precinct_refs = new kd_precinct_ref[(int)num_precincts];
...
//ohhan03
int
kdu_resolution::get_comp_idx()
{
	return state->rescomp->comp_idx;
}
                                       ...
                                       bool parse_if_necessary)
{
  ...
  return (precinct==NULL)?0:(precinct->num_packets_read);
}
                          ...
                          kdu_thread_env *env)
{
  ...
  //modified by JYH
  result->Qmode = codestream->Qmode;
  result->mapon = codestream->mapon;
  result->p_fname = codestream->p_fname;
  result->Qquality = codestream->Qquality;
  result->vtnums = codestream->vtnums;
  result->mapbuffer = codestream->mapbuffer;
  result->mapheight = codestream->mapheight;
  result->mapwidth = codestream->mapwidth;
  result->extralayer = codestream->extralayer;
  ...
  kd_precinct_size_class::augment_free_list()
{
  ...
  if (elt == NULL)
    { KDU_ERROR(e,39); e <<
        KDU_TXT("Heap exhausted.  Unable to allocate sufficient "
        "memory for code-block state information.");
    }
  elt->size_class = this;
  elt->next = free_list;
  free_list = elt;
  ...
  kd_precinct_server::get(int max_blocks, int num_subbands)
{
  ...
  if (scan == NULL)
    {
      scan =
        new kd_precinct_size_class(max_blocks,num_subbands,this,buf_server);
