...
class kd_tlm_generator {
  public: // Member functions
    ...
    bool init(int num_tiles, int max_tparts);
      /* Call this function after construction or `clear' in order to
         initialize the object in preparation for generating TLM marker
         segments.  If the number of tiles or tile-parts identified here
         cannot be accommodated by a legal set of TLM marker segments, the
         function returns false and `exists' will continue to return false.
    ...
    void clear()
      {
        num_tiles = max_tparts = num_elts = 0;
        tile_data_bytes = 0;
        if (elts != NULL)
...
class kd_codestream_comment {
  public: // Member functions
    kd_codestream_comment()
      { readonly=false; max_chars=num_chars=0; text_buf=NULL; next=NULL; }
    ~kd_codestream_comment()
      { if (text_buf != NULL) delete[] text_buf; }
    void set_text(int length, kdu_byte *data);
      /* This function is used only when parsing comment markers from a
         code-stream, where the comment marker indicates that it contains
         text.  The `data' array contains `length' characters, starting from
         the first byte in the body of the comment marker.  The array of
         characters may or may not be terminated with a null character.  If
         not, a null character is automatically appended to the internal
         representation.
    ...
    int write_marker(kdu_output *out, int force_length=0);
      /* If `out' is NULL, the marker segment is ony simulated.  In
         ...
         to occupy an exact number of bytes.  If necessary, the text body of
         the marker segment will be truncated, or padded to the specified
         length.  A non-zero `force_length' value may not be less than 6,
         which is the length of the comment marker header.
            This function leaves the object in the `read-only' state.
      */
  private: // Data
    friend class kdu_codestream_comment;
    bool readonly; // True if you are not allowed to write to the text buffer
    int max_chars; // `text_buf' can hold this many chars, plus a null
    int num_chars; // Number of characters written to `text_buf'.
    char *text_buf; // Holds comment text.
  public: // Links
...
struct kd_tile { // State structure for the "kdu_tile" interface
  ...
  public: // Flags and other State Variables
    ...
    bool needs_reinit; // Set inside `reset' to indicate the need for
