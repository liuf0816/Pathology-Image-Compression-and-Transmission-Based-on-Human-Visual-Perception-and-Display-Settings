  ...
  kd_tlm_generator::init(int num_tiles, int max_tparts)
{
  ...
  tlm_bytes = 0;
  do {
      int tlm_elts = (65535-4) / 6;
      if (tlm_elts > elts_left)
        tlm_elts = elts_left;
      elts_left -= tlm_elts;
      tlm_bytes += 6 + 6*tlm_elts;
      z_tlm++;
  ...
  kd_tlm_generator::add_tpart_length(int tnum, kdu_long length)
{
  ...
  if (length != (kdu_long) elts[elt_ctr].length)
    { KDU_ERROR(e,10); e <<
        KDU_TXT("Attempting to write TLM (tile-part length) data "
        "where one tile-part's length cannot be represented as an "
  ...
  kd_tlm_generator::write_dummy_tlms(kd_compressed_output *out)
{
  ...
  do {
      ...
      if (tlm_elts > elts_left)
        tlm_elts = elts_left;
      elts_left -= tlm_elts;
      check_tlm_bytes += 6 + 6*tlm_elts;
      out->put(KDU_TLM);
      out->put((kdu_uint16)(4+6*tlm_elts));
      out->put((kdu_byte) z_tlm);
      out->put((kdu_byte) 0x60); // Style with 2-byte tnum + 4-byte length
      z_tlm++;
      out->write(marker_body,6*tlm_elts);
    } while (elts_left > 0);
  ...
  while (elts_left > 0)
    {
      if (tlm_elts_left == 0)
        { // Open a new marker segment
          assert(z_tlm <= 255);
          tlm_elts_left = (65535-4) / 6;
          backtrack -= 6; // For the TLM header already written
          z_tlm++;
        }
      ...
      if (tlm_elts_left == 0)
        { // Open a new marker segment
          ...
          if (tlm_elts_left > elts_left)
            tlm_elts_left = elts_left;
          out.put(KDU_TLM);
          out.put((kdu_uint16)(4+6*tlm_elts_left));
          out.put((kdu_byte) z_tlm);
          out.put((kdu_byte) 0x60); // Style with 2-byte tnum + 4-byte length
          z_tlm++;
        }
  ...
  kd_tpart_pointer_server::add_tlm_marker(kd_marker &copy_source)
{
  ...
  kd_tlm_marker_list *elt =
    new kd_tlm_marker_list(copy_source);
  kdu_byte *data = elt->get_bytes();
  elt->znum = data[0];
...
void
  kd_codestream_comment::set_text(int length, kdu_byte *data)
{
  assert(!readonly);
  readonly = true;
  if (length <= 0)
  ...
  if (max_chars < length)
    {
      max_chars = length;
      if (text_buf != NULL)
        { delete[] text_buf; text_buf = NULL; }
      text_buf = new char[max_chars+1];
    }
  num_chars = length;
  memcpy(text_buf,data,(size_t) length);
  if (text_buf[length-1] != '\0')
    text_buf[length] = '\0';
  else
    num_chars--;
}
  ...
  if (force_length != 0)
    {
      ...
      if (force_length < 0)
        force_length = 0;
      if (force_length < num_chars)
        {
          num_chars = force_length;
          text_buf[num_chars] = '\0';
        }
      else
        pad_chars = force_length - num_chars;
    }
  if (out != NULL)
    {
      out->put(KDU_COM);
      out->put((kdu_uint16)(4+num_chars+pad_chars));
      out->put((kdu_uint16) 1); // Latin values
      out->write((kdu_byte *) text_buf,num_chars);
      for (int i=0; i < pad_chars; i++)
        out->put((kdu_byte) 0);
    }
  return 6+num_chars+pad_chars;
}
  ...
  kdu_codestream_comment::get_text()
{
  if (state == NULL)
    return NULL;
  else if (state->text_buf == NULL)
    return "";
  else
    return state->text_buf;
}
  ...
  kdu_codestream_comment::put_text(const char *string)
{
  if ((state == NULL) || state->readonly)
    return false;
  int len = state->num_chars + (int) strlen(string);
  if (len > 65531)
  ...
  if (len > state->max_chars)
    {
      state->max_chars = len + state->max_chars + 80;
      char *old_buf = state->text_buf;
      state->text_buf = new char[state->max_chars+1];
      if (old_buf == NULL)
        state->text_buf[0] = '\0';
      else
        {
          strcpy(state->text_buf,old_buf);
          delete[] old_buf;
        }
    }
  if (len > state->num_chars)
    strncat(state->text_buf,string,(size_t)(len-state->num_chars));
  state->num_chars = len;
  return true;
}
  ...
  for (stage_num=0; stage_num < num_stages; stage_num++)
    {
      ...
      if ((stage_num == (num_stages-1)) &&
        ...
        stage->num_outputs = num_output_components;
      stage->output_comp_info =
        new kd_output_comp_info[stage->num_outputs];
      for (b=0; b < stage->num_blocks; b++)
        {
          kd_mct_block *block = stage->blocks + b;
  ...
  if (out != NULL)
    {
      ...
      if (rc == NULL)
        rc = global_rescomps =
          new kd_global_rescomp[33*num_components];
      for (d=0; d < 33; d++)
                                     ...
                                     kdu_long fragment_tile_bytes_generated)
{
  ...
  idx_lim.x = lim.x / tsize.x;
  idx_lim.y = lim.y / tsize.y;
  if ((min.x != idx_min.x*tsize.x) || (min.y != idx_min.y*tsize.y) ||
      (lim.x != idx_lim.x*tsize.x) || (lim.y != idx_lim.y*tsize.y))
    { KDU_ERROR_DEV(e,31); e <<
        KDU_TXT("The fragment region supplied to "
        "`kdu_codestream::create' does not correspond to a whole number "
        "of tiles.  Check your dimensions again, remembering to take "
        "account of any non-zero tile or image origin.");
    }
  ...
  for (prev=NULL, scan=comhead; scan != NULL; prev=scan, scan=scan->next)
    {
      ...
      if (strncmp(com.get_text(),"Kdu-Layer-Info: ",
                  strlen("Kdu-Layer-Info: ")) == 0)
        break;
    }
  if (scan != NULL)
    {
  ...
  do {
      ...
      if (marker->get_code() == KDU_PPM)
      ...
      else if (marker->get_code() == KDU_COM)
        {
          ...
          if ((len > 2) && (dat[0] == 0) && (dat[1] == 1))
            {
              if (comtail == NULL)
                comhead = comtail = new kd_codestream_comment;
              else
                comtail = comtail->next = new kd_codestream_comment;
              comtail->set_text(len-2,dat+2);
            }
        }
      else
  ...
  if (comscan == NULL)
    {
      ...
      comscan->set_text((int) strlen(KDU_IDENTIFIER),
                        (kdu_byte *) KDU_IDENTIFIER);
      if (comtail == NULL)
...
void 
  kdu_codestream::set_vt_params(int vt_nums,int Qmode,double *Qvalues,double *Rvalues, bool fastmode, double *VTvalues) {
	state->Qmode = Qmode;
	state->vtnums = vt_nums;
	if (fastmode == true) {
		state->Qmode = 4;
		state->Qquality = VTvalues;
	}
	else {
		if (Qmode == 1)
		{
			state->Qquality = Qvalues;
		}
		else if (Qmode == 2)
		{
			state->Qquality = Rvalues;
		}
		else
		{
			...
			state->Qquality = result;
		}
	}

	//for (int vt_num = 0; vt_num < state->vtnums + 1; vt_num++) {
	//	printf("%lf\n", state->Qquality[vt_num]);
	//}

}

void
  kdu_codestream::set_map_fname(char *pfname, unsigned char *buffer, int height, int width,bool extralayer)
{
	if ( pfname != NULL ) {
		state->p_fname = pfname;
		state->mapon = true;
		state->mapbuffer = buffer;
		state->mapheight = height;
		state->mapwidth = width;
		state->extralayer = extralayer;
		//printf("mapname=%s\n", state->p_fname);
	}
	else {
		state->mapon = false;
