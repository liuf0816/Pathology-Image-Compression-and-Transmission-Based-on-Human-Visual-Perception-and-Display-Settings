  ...
  kd_synthesis::pull(kdu_line_buf &line, kdu_thread_env *env)
{
  ...
  if ((num_vert_steps == 0) || unit_height)
    { // No vertical synthesis
      ...
      horizontal_synthesis(vline_out,vsub_parity,env);
                  ...
                  if (y_next_in[vsub_parity] <= y_max_in[vsub_parity])
                    {
                      ...
                      horizontal_synthesis(vline_in,vsub_parity,env);
                      ...
                      if (use_shorts)
                        {
                          ...
                          perform_synthesis_lifting_step(step,bf,
                                      ...
                                      coset_width_out[c],coset_off_out[c]);
                        }
                      else
                        {
                                     ...
                                     kdu_thread_env *env)
{
  ...
  if (num_hor_steps == 0)
    return;
  if (unit_width && reversible && (x_min_in & 1))
    {
      if (!use_shorts)
