  ...
  kdsx_image_entities::add_entity(kdu_int32 idx)
{
  ...
  if (max_entities == num_entities)
    {
      max_entities += 1+num_entities;
      kdu_int32 *tmp = new kdu_int32[max_entities];
                         ...
                         kdu_long fpos_lim)
{
  ...
  is_placeholder =
    (total_length > (kdu_long) max_size) || create_stream_equivalent ||
    ((contents_length > 0) &&
  ...
  if (parent != NULL)
  ...
  else
    {
      ...
      scope->sequence = KDU_INT32_MAX; // Until proven otherwise
      ...
      for (int n=0; n < (int) num; n++)
        {
          ...
          if (Rtyp == 1)
            { // Elliptical region; `pos' indicates centre, rather than corner
              rect.size.x |= 1;
              rect.size.y |= 1;
              rect.pos.x -= (int)(rect.size.x>>1);
              rect.pos.y -= (int)(rect.size.y>>1);
            }
          if (!(scope->flags & KDS_METASCOPE_HAS_REGION_SPECIFIC_DATA))
            {
  ...
  else if (last_box_type != jp2_association_4cc)
    {
      if (!(scope->flags & KDS_METASCOPE_HAS_IMAGE_SPECIFIC_DATA))
        scope->flags |= KDS_METASCOPE_HAS_GLOBAL_DATA;
      else if (!(scope->flags & KDS_METASCOPE_HAS_REGION_SPECIFIC_DATA))
        scope->flags |= KDS_METASCOPE_HAS_IMAGE_WIDE_DATA;
    }
      ...
      if (!create_stream_equivalent)
        {
          ...
          if (!jp2_is_superbox(last_box_type))
          ...
          else
            {
              ...
              kdu_int32 asoc_flag_mask = 0xFFFFFFFF & // Mask will be used to
                ~(KDS_METASCOPE_HAS_GLOBAL_DATA |     // remove inappropriate
                  KDS_METASCOPE_HAS_IMAGE_WIDE_DATA); // flags from asoc boxes
                  ...
                  if (sub.exists())
                    {
                      ...
                      if (last_box_type == jp2_association_4cc)
                        {
                          if (elt == phld)
                            { // Make sure first sub-box scope gets reflected
                              // to its siblings.
                              inherit_child_scope(entities,elt);
                            }
                          else
                            asoc_flag_mask |= elt->scope->flags;
                        }
                    }
                  sub.close();
                } while (sub.open(box));
              tail->is_last_in_bin = true;
              if ((last_box_type==jp2_association_4cc) && (phld->next != NULL))
                {
                  scope->flags &= asoc_flag_mask;
                  phld->scope->flags &= asoc_flag_mask;
                }
  ...
  if (data != NULL)
    fpos = 0; // Won't be needing file position any more, since data is local
  ...
  if (have_data)
    {
      if ((length < 0) || (length > (1<<16)))
        { kdu_error e; e << "Cache representation of meta-data structure "
          ...
          "of meta-data specially synthesized as a streaming equivalent for "
          "an original box."; }
...
void
  kdsx_stream::deserialize(FILE *fp)
{
  int i;
  kdu_byte *bp, inbuf[KDSX_OUTBUF_LEN];
  ...
  if (fname == NULL)
    {
      ...
      if ((fname != parent_filename) && (!is_absolute))
        { // Need an expanded filename
          expanded_filename =
            new char[strlen(fname)+strlen(parent_filename)+2];
                   ...
                   bool cache_exists, kdu_long sub_start, kdu_long sub_lim)
{
  ...
  // Finish up by installing the `per_client_cache' value and
  // the file names for each code-stream.
  kdsx_stream *str;
  for (str=stream_head; str != NULL; str=str->next)
    {
      ...
      if (str->url_idx == 0)
      ...
      else
        {
          ...
          str->target_filename = drefs.get_url(str->url_idx);
                               ...
                               kd_serve *thread_handle)
{
  ...
  bool been_through_once = false; // Add to waiting count only on first pass
  acquire_lock();
  while (!all_available)
    {
      all_available = true;
  ...
  // By the time we get here, all requested codestreams are unlocked.  Now
  // it is safe to lock them all.  If we had locked them incrementally as
  // we went, we could have created a deadlock scenario.
  for (n=0; n < num_streams; n++)
    {
      index = stream_indices[n];
      if ((index < 0) || (index > codestream_range[1]) ||
          ((str = stream_refs[index]) == NULL))
        continue;
      assert((str->locking_thread_handle == NULL) &&
             (str->num_waiting_for_lock > 0));
      str->num_waiting_for_lock--;
      ...
      while (box.exists())
        {
          ...
          elt->create(NULL,NULL,&box,phld_threshold,last_used_bin_id,
                      num_codestreams,num_jpch,num_jplh,sub_lim);
