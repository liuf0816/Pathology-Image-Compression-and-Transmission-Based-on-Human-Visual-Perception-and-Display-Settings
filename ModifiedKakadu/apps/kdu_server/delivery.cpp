/*****************************************************************************/
// File: delivery.cpp [scope = APPS/SERVER]
// Version: Kakadu, V6.1
                                             ...
                                             float min_rtt_target)
{
  ...
  clock_t max_bunching = (clock_t)(CLOCKS_PER_SEC*max_rtt_target/2);
                                 ...
                                 const char *content_type)
{
  ...
  if (((holding_bytes+window_bytes) > window_threshold) &&
    ...
    { // We will need to block.
      ...
      kdu_uint32 wait_miliseconds = (wait_ticks * 1000) / CLOCKS_PER_SEC;
      WSAWaitForMultipleEvents(1,&source_event,TRUE,wait_miliseconds,FALSE);
      acquire_lock();
      source_event = NULL;
      push_waiting = false;
    }
  ...
  if (((holding_bytes+window_bytes) <= window_threshold) ||
      (holding_head == NULL))
    {
      ...
      if (was_idle)
        WSASetEvent(service_event); // Wake up service thread
      push_succeeded = true;
    }
  release_lock();
  ...
  kd_tcp_transmitter::service_queue()
{
  ...
  kdu_byte *ack;
  kd_queue_elt *qp;
  clock_t now = clock() - base_time; // Get adjusted reference time.
  ...
  try { // So we can unlock the mutex in the event of an exception
      ...
      while ((ack = channel->read_raw(false,8)) != NULL)
        {
          ...
          total_rtt_events++;
          float interval_time = (1.0F / CLOCKS_PER_SEC) *
            (float)(1 + now - qp->interval_start_time);
          float window_time = (1.0F / CLOCKS_PER_SEC) *
      ...
      if (push_waiting && ((window_bytes+holding_bytes) <= window_threshold))
        WSASetEvent(source_event); // Unblock `push_chunk'
      ...
      if ((now >= delivery_gate) && (holding_head != NULL) &&
        ...
        { // Can transmit pending chunk; transmission window not yet full
          ...
          delivery_gate += 1 + (clock_t)
            (qp->chunk->num_bytes / max_bytes_per_second * CLOCKS_PER_SEC);
  ...
  if (window_bytes > window_threshold)
    ...
    { // Must wait for delivery gate to be reached
      if (delivery_gate <= now)
        ...
        return_value = (int)
          (((float)(delivery_gate-now)) * (1000.0F/CLOCKS_PER_SEC));
    }
      ...
      if (now < disconnect_gate)
        {
          ...
          clock_t wait_ticks = disconnect_gate - now;
          kdu_uint32 wait_miliseconds = (wait_ticks * 1000) / CLOCKS_PER_SEC;
          WSAWaitForMultipleEvents(1,&source_event,TRUE,
                                   wait_miliseconds,FALSE);
          acquire_lock();
          source_event = NULL;
          now = clock() - base_time;
        }
  ...
  try { // So we can unlock the mutex in the event of an exception
      ...
      if ((qp != NULL) && (now >= delivery_gate) &&
          ((!waiting_for_chunk) || (qp != holding_tail)))
        {
          ...
          // Determine the next delivery gate.
          delivery_gate += 1 + (clock_t)
            (qp->started_bytes / max_bytes_per_second * CLOCKS_PER_SEC);
          if ((delivery_gate+bunching_ticks) < now)
            delivery_gate = now - bunching_ticks;
...
static unsigned int __stdcall
  kd_delivery_manager_start_proc(void *param)
{
  ...
  kd_delivery_manager::start()
{
  ...
  h_thread = (HANDLE)
    _beginthreadex(NULL,0,kd_delivery_manager_start_proc,this,0,&thread_id);
  ...
  do {
      ...
      if (wait_miliseconds < 0)
        { // Wait until service event is signalled
          WSAWaitForMultipleEvents(1,&h_event,FALSE,WSA_INFINITE,FALSE);
        }
      else if (wait_miliseconds > 0)
        {
          WSAWaitForMultipleEvents(1,&h_event,FALSE,
                                   (kdu_uint32)(wait_miliseconds+10),FALSE);
        }
  ...
  kd_delivery_manager::get_tcp_transmitter(kd_tcp_channel *channel)
{
  ...
  num_transmitters++;
  WSASetEvent(h_event); // Wake up delivery thread.
  release_lock();
  return transmitter;
}
