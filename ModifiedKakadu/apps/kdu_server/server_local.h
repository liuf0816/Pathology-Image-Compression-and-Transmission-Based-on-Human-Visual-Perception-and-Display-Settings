  ...
  public: // Member functions
    kd_transmitter()
      { /* Basic initialization only.  You must call `configure' afterwards. */
        ...
        bunching_ticks = (clock_t)(CLOCKS_PER_SEC*0.5F);
    ...
    virtual void configure(kd_tcp_channel *channel,
      ...
      { /* The `channel' supplied here is used to send all outgoing data
        ...
        if (max_connection_seconds > 0.0F)
          disconnect_gate = (clock_t)(CLOCKS_PER_SEC*max_connection_seconds);
      }
  ...
  public: // Functions for monitoring transmission statistics and times.
    kdu_uint32 get_time_left()
      { /* Returns the amount of time left before the client connection
        ...
        return (int)(((float)(disconnect_gate-now))*(1000.0F/CLOCKS_PER_SEC));
      }
    int get_total_transmitted_bytes() { return total_transmitted_bytes; }
    float get_connected_seconds()
      { clock_t ticks = clock() - base_time;
        return ((float) ticks) / CLOCKS_PER_SEC; }
    float get_active_seconds()
      { clock_t ticks = clock() - base_time;
        if (idle_start != 0) ticks = idle_start;
        return ((float)(ticks - idle_ticks)) / CLOCKS_PER_SEC; }
  protected: // Helper functions
    void acquire_lock() { WaitForSingleObject(h_mutex,INFINITE); }
    void release_lock() { ReleaseMutex(h_mutex); }
    ...
    float get_average_rtt()
      {
        if (total_rtt_events == 0)
          ...
          return ((float) total_rtt_ticks) / (total_rtt_events*CLOCKS_PER_SEC);
      }
  private: // Helper functions
...
class kd_delivery_manager {
  ...
  private: // Data
    ...
    WSAEVENT h_event; // Local event passed to `kd_deliver::serice_queue'
    HANDLE h_thread;
...
class kd_synchronized_serve_target : public kdu_servex {
  public: // Member functions
    ...
    void acquire_lock() { WaitForSingleObject(h_mutex,INFINITE); }
    void release_lock() { ReleaseMutex(h_mutex); }
    void wait_event()   { WaitForSingleObject(h_event,INFINITE); }
    void pulse_event()  { PulseEvent(h_event); }
  private: // Data
    HANDLE h_mutex;
    HANDLE h_event;
  };
...
class kd_source_thread : private kdu_serve {
  ...
  private:
    ...
    void wait_event(kdu_uint32 timeout_milliseconds=INFINITE)
      { WaitForSingleObject(h_event,timeout_milliseconds); }
    void signal_event() { SetEvent(h_event); }
    void reset_event() { ResetEvent(h_event); }
...
class kd_source_manager {
  public: // Member functions
    ...
    void request_shutdown()
      { shutdown_requested = true; SetEvent(shutdown_event); }
