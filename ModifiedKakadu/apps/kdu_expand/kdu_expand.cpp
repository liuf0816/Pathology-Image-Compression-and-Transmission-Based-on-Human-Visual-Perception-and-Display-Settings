  ...
  print_usage(char *prog, bool comprehensive=false)
{
  ...
  if (comprehensive)
    out << "\tYou can use this argument to adjust the way in which sample "
           "values are recorded in the output file(s).  It makes very "
           "little sense to force the data to be recorded with less "
           "precision than that recorded in the codestream header, so "
           "file writing modules are unlikely to support forced precision "
           "values which are smaller than those identified by the "
           "`Sprecision' parameter attribute (or `Mprecision' if using "
           "Part-2 multi-component transforms).  If the forced precision "
           "value supplied for an image component is greater than that "
           "which would normally be used, the data is effectively written "
           "into the least significant bits of each word, but the sample "
           "values are not clipped; nor are the unused bits masked off.  "
           "This means that values which exceed the nominal precision can "
           "be preserved.\n"
           ...
           "\"-fprec 8\", the sample values are written into the least "
           "significant 4 bits and only clipped to the range which can "
           "be accommodated by each 8-bit byte.  If you were to save with "
           "\"-fprec 6\", the least significant 2 bits in each written byte "
           "would be 0, and sample values up to 4 times as large as the "
           "nominal range can be preserved.\n"
           ...
           "\"-fprec 16\", for example, to force each sample to be saved "
           "into its own 16-bit word; of course, the TIFF file will then "
...
static kde_file_binding *
  parse_simple_args(kdu_args &args, char * &ifname,
                    ...
                    int &cpu_iterations, bool &simulate_parsing,
                    bool &mem, bool &stats, bool &quiet)
...
int main(int argc, char *argv[])
{
  ...
  std::ostream *record_stream;
  float max_bpp;
  bool no_seek, simulate_parsing, want_alpha;
  ...
  kde_file_binding *outputs =
    parse_simple_args(args,ifname,record_stream,max_bpp,
                      ...
                      num_threads,double_buffering_height,cpu_iterations,
                      simulate_parsing,mem,stats,quiet);
  ...
  if (resolution.exists())
    extract_jp2_resolution_info(idims,resolution,layer_size,transpose);
  if (channels.exists())
  ...
  for (oscan=outputs; oscan != NULL; oscan=oscan->next)
    {
      ...
      oscan->writer =
        kdu_image_out(oscan->fname,idims,num_output_channels,flip);
