  ...
  print_usage(char *prog, bool comprehensive=false)
{
  ...
  if (comprehensive)
    out << "\tTo avoid over complicating this simple demonstration "
           "application, input video must be supplied as a VIX file.  VIX "
           "is a trivial non-standard video file format, consisting of a "
           "plain ASCI text header, followed by raw binary data.\n"
...
static FILE *
  ...
  /* Opens the VIX input file, reading its header and returning a file
     pointer from which the sample values can be read. */
{
    ...
    if (strcmp(buffer,">VIDEO<") == 0)
    ...
    else if (strcmp(buffer,">COLOUR<") == 0)
      {
        if ((!read_token(fp,buffer,64)) ||
          ...
          { kdu_error e; e << "Malformed \">COLOUR<\" tag found in "
            "VIX input file.  Tag requires a single token, with one of the "
            "strings, \"RGB\" or \"YCbCr\"."; }
    ...
    else if (strcmp(buffer,">IMAGE<") == 0)
      {
        ...
        if ((!read_token(fp,buffer,64)) ||
          ...
          { kdu_error e; e << "Malformed  \">IMAGE<\" tag found in VIX "
            "input file.  Third token in tag must hold the number of MSB's "
            "used in each sample word, a quantity in the range 1 through to "
            "the number of bits in the sample word."; }

        if ((!read_token(fp,buffer,64)) ||
            (strcmp(buffer,"little-endian") && strcmp(buffer,"big-endian")))
          { kdu_error e; e << "Malformed \">IMAGE<\" tag found in VIX "
            "input file.  Fourth token in tag must hold one of the strings "
        ...
        if ((!read_token(fp,buffer,64)) ||
          ...
          { kdu_error e; e << "Malformed \">IMAGE<\" tag found in VIX "
            ...
            "values for the width, height and number of components in each "
            "frame, respectively."; }
        ...
        for (int c=0; c < components; c++)
          {
            ...
            if ((!read_token(fp,buffer,64)) ||
                ...
                (sub_y < 1) || (sub_y > 255))
              { kdu_error e; e << "Malformed \">IMAGE<\" tag found in VIX "
                "input file.  Horizontal and vertical sub-sampling factors "
                "in the range 1 to 255 must appear for each image "
                "component."; }
