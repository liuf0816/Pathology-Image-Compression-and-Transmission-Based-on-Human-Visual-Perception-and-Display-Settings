      ...
      while (sample_count > 0)
        {
          if (tail == NULL)
            ...
            tail = tail->next = new mj_sample_size_store;
  ...
  while (chunk_count > 0)
    {
      if (tail == NULL)
        ...
        tail = tail->next = new mj_chunk_offset_store;
      tail->num_elts = tail->remaining_elts;
  ...
  for (; entry_count > 0; entry_count--, prev_first_chunk=first_chunk)
    {
      ...
      if (tail == NULL)
      ...
      else
        {
          ...
          tail = tail->next = new mj_samples_per_chunk;
        }
  ...
  for (; entry_count > 0; entry_count--)
    {
      if (tail == NULL)
        ...
        tail = tail->next = new mj_ticks_per_sample;
