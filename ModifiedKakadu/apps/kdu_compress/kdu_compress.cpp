  ...
  print_usage(char *prog, bool comprehensive=false)
{
  ...
  if (comprehensive)
    out << "\tOne or more input files. If multiple files are provided, "
           ...
           "components.\n"
           "\t   Currently accepted image file formats are: TIFF, " 
           "RAW (big-endian), RAWL (little-endian), BMP, PBM, PGM and PPM, "
           "as determined by the file suffix.  For raw files, the sample "
           ...
           "big-endian for files with the suffix \".raw\", but little-endian "
           "for files with the suffix \".rawl\" -- be careful to get this "
           "right.  Also, with raw files, the dimensions, precision and "
           "signed/unsigned characteristics must be provided separately using "
           "`Sdims' (or any other appropriate combination of SIZ-type "
           "parameters), `Sprecision' and `Ssigned'.\n"
  ...
  if (comprehensive)
    out << "\tName of file to receive the compressed code-stream.  If the "
           ...
           "arguments.\n";
  out << "-fprec <comp-0 precision>[,<comp-1 precision>[...]]\n";
  if (comprehensive)
    out << "\tYou can use this argument to adjust the way in which sample "
           ...
           "of the least significant bits of the values recorded in the "
           "input image file(s); in this case, you would typically supply a "
  ...
  if (comprehensive)
    out << "\tYou may use this to explicitly specify a JP2 compatible "
           ...
           "consisting of one of 5 colour space names, possibly followed "
...
read_image(char *fname, int &num_components, int &height, int &width)
{
	...
	unsigned char*buffer = new unsigned char[num_samples];
	in.read((char *)buffer, num_samples);
	return buffer;
}
...
double VTcal(int orient, int idx, int level, double Q, double r) {

	//T values
	double var[11][3] = { {4.6333,0.2392,0.2980},{8.6333,0.3698,0.4652},{8.0645,0.2984,0.3781},{11.9182,0.6098,0.8657},{8.8622,0.4591,0.7098},{11.0231,0.7799,1.3189},{6.8845,0.5934,1.0700},{7.8518,0.8880,1.5830},{4.5779,0.5901,1.0479},{5.5305,0.7611,1.3772},{14.2401,2.2969,3.7418} };
	...
	if (orient != 0) {
		if (mode == 1) {
			vt = m*t02tb(mask*T, Q, 2.0, 1, sig2, level, 0.1845, 600, r);
		}
		else {
			vt = mask*T / Q;
		}
		...
		if (mode == 1) {
			vt = m*t02tb(mask*T, Q, 2.0, 1, sig2, 5, 0.1845, 600, r);
		}
		else {
			vt = mask*T / Q;
		}
...
double * GenVT(int numlayer, int Qmode, double *Qvalues,double *Rvalues,bool mapon) {
	int VTnumber,vtlayer;
	if (mapon == false) {
		vtlayer = numlayer;
	}
	else {
	...
	VTnumber = vtlayer * 3 * 11;
	
	double *VTs = new double[VTnumber];

	if (Qmode == 1) {
		for (int i_vt = 0; i_vt < vtlayer ; i_vt++) {
			for (int level = 1; level <= 5; level++) {
				for (int j = 0; j < 3; j++) {

					VTs[3 * (2 * level - 1) + j + 33 * i_vt] = VTcal(1, j, level, Qvalues[i_vt], Qvalues [vtlayer]);

					VTs[3 * (2 * level - 2) + j + 33 * i_vt] = VTcal(3, j, level, Qvalues[i_vt], Qvalues[vtlayer]);

					if (level == 5) {
						VTs[3 * (2 * level) + j + 33 * i_vt] = VTcal(0, j, level, Qvalues[i_vt], Qvalues[vtlayer]);
					}
		...
		for (int i_vt = 0; i_vt < vtlayer ; i_vt++) {
			for (int level = 1; level <= 5; level++) {
				for (int j = 0; j < 3; j++) {
					...
					VTs[3 * (2 * level - 1) + j + 33 * i_vt] = VTcal(1, j, level, Rvalues[vtlayer], Rvalues[i_vt]);

					VTs[3 * (2 * level - 2) + j + 33 * i_vt] = VTcal(3, j, level, Rvalues[vtlayer], Rvalues[i_vt]);

					if (level == 5) {
						VTs[3 * (2 * level) + j + 33 * i_vt] = VTcal(0, j, level, Rvalues[vtlayer], Rvalues[i_vt]);
					}
		...
		for (int level = 1; level <= 5; level++) {
			for (int j = 0; j < 3; j++) {
				VTs[3 * (2 * level - 1) + j] = VTcal(1,j,level,1,1);

				VTs[3 * (2 * level - 2) + j] = VTcal(3, j, level, 1, 1);

				if (level == 5) {
					VTs[3 * (2 * level) + j] = VTcal(0, j, level, 1, 1);
				}
...
static kdc_file_binding *
  parse_simple_args(kdu_args &args, char * &ofname,
                    ...
                    int &double_buffering_height, int &cpu_iterations,
                    bool &mem, bool &quiet, char * &pfname, bool &fastmode,bool &extralayer)
     ...
     will involve only one thread. */
{
  ...
  if (args.find("-Pmap") != NULL)
  {
	  char *string = args.advance();
	  if (string == NULL)
	  {
		  kdu_error e; e << "\"-Pmap\" argument requires a file name!";
	  }
	  pfname = new char[strlen(string) + 1];
	  strcpy(pfname, string);
	  args.advance();
  }//modified by JYH

  if (args.find("-fastmode") != NULL)
  {
	  args.advance();
	  fastmode = true;
  }
  if (args.find("-extralayer") != NULL)
  {
	  args.advance();
	  extralayer = true;
  }
  ...
  parse_forced_precisions(kdu_args &args, kdu_image_dims &idims)
{
  ...
  do {
    int val = 0;
    if ((string == NULL) || (sscanf(string,"%d",&val) != 1) || (val < 0))
      { kdu_error e; e << "Malformed `-fprec' argument.  Expected a comma "
        "separated list of non-negative forced precision values."; }
    idims.set_forced_precision(comp_idx++,val);
...
static kdu_long *
  assign_layer_bytes(kdu_args &args, kdu_codestream &codestream,
                     int &num_specs, int &Qmode,bool mapon,bool extralayer)
     ...
     layers on the fly. */
{
  ...
  if ( Qmode != 0 ) {
	  //printf("slope_specs=%i\n", slope_specs);
	  num_specs = slope_specs;
...
/*****************************************************************************/
//modified by JYH
static double *assign_layer_jndr(kdu_args &args, int num_specs_new,bool extralayer) {
	int n;
	int num_specs;
	if (extralayer == true) {
		num_specs = num_specs_new - 1;
	}
	else
	{
		num_specs = num_specs_new;
	}
	double *result = new double[num_specs + 1];
	for (n = 0; n < num_specs+1; n++)
		result[n] = 0;
	if (args.find("-JNDR") == NULL)
		return result;
	char *string = args.advance();
	if (string == NULL)
	{
		kdu_error  e; e << "The `-JNDR' argument must be followed by a "
			"comma-separated list of jnd values.";
	}
	for (n = 0; (n < num_specs+1) && (string != NULL); n++)
	{
		char *delim = strchr(string, ',');
		if (delim != NULL)
		{
		...
		double val;
		if ((sscanf(string, "%lf", &val) != 1) || (val < 0) )
		{
			kdu_error e; e << "The `-JNDR' argument must be followed by a "
				"comma-separated  list of integer distortion-length jnd values, ";
		}
		result[n] = (double)val;
		string = delim;
	}
...
static double *assign_layer_rjnd(kdu_args &args, int num_specs_new,bool extralayer) {
	int n;
	int num_specs;
	if (extralayer == true) {
		num_specs = num_specs_new - 1;
	...
	if (string == NULL)
	{
		kdu_error  e; e << "The `-RJND' argument must be followed by a "
			"comma-separated list of jnd values.";
	}
	for (n = 0; (n < num_specs + 1) && (string != NULL); n++)
	{
...
static kdu_uint16 *
  assign_layer_thresholds(kdu_args &args, int num_specs, int Qmode,bool extralayer)
     ...
     an explicit request for particular distortion-length slope thresholds. */
{
  ...
  if (Qmode == 0 && extralayer ==false) {
	  for (n = 0; n < num_specs; n++)
		  result[n] = 0;
  }
  else {
	  for (n = 0; n < num_specs; n++)
		  result[n] = num_specs - n;
  }
  if (args.find("-slope") == NULL)
    return result;
  char *string = args.advance();
...
int main(int argc, char *argv[])
{
  ...
  int num_threads, double_buffering_height, cpu_iterations;
  char *ofname;
  char *pfname;//modified by jyh
  bool mapon=false;
  bool fastmode=false;
  ...
  kdc_file_binding *inputs =
    parse_simple_args(args,ofname,record_stream,transpose,vflip,hflip,
                      ...
                      cpu_iterations,mem,quiet,pfname,fastmode,extralayer);
  ...
  int c;
  int num_source_components = 0; // Will be derived from the source files
  ...
  for (iscan=inputs; iscan != NULL; iscan=iscan->next)
    {
      ...
      iscan->first_comp_idx = num_source_components;
      iscan->reader =
  ...
  int Qmode = 0; //modified by JYH
  if (mapon == false) {
	  extralayer = false;
  }
  kdu_long *layer_bytes =
    assign_layer_bytes(args,codestream,num_layer_specs,Qmode,mapon,extralayer);
  kdu_uint16 *layer_thresholds =
    assign_layer_thresholds(args,num_layer_specs,Qmode,extralayer);

  //if (mapon == true) {
	 // printf("layer_numer=%i\n", num_layer_specs);
  //}
  double *Qvalues= assign_layer_jndr(args, num_layer_specs,extralayer);
  double *Rvalues = assign_layer_rjnd(args, num_layer_specs,extralayer);
  ...
  if (fastmode == true) {
	  //printf("fastmode is using\n");
	  VTvalues = GenVT(num_layer_specs, Qmode, Qvalues, Rvalues, extralayer);
	  //for (int i = 0; i < 33 * num_layer_specs; i++)
		 // printf("VT[%d]=%lf\n",i,VTvalues[i]);
  }
  else {
	  VTvalues = NULL;
  ...
  codestream.set_vt_params(num_layer_specs, Qmode, Qvalues, Rvalues,fastmode,VTvalues);
  ...
  int mapnum=0;
  if (mapon == true) {
	  unsigned char *mapbuffer = read_image(pfname, mapnum, mapheight, mapwidth);
	  codestream.set_map_fname(pfname, mapbuffer, mapheight, mapwidth,extralayer);
  }
  else {
	  unsigned char *mapbuffer=NULL;
	  codestream.set_map_fname(pfname, mapbuffer, mapheight, mapwidth,extralayer);
  }
  ...
  //for (int vt_num = 0; vt_num < num_layer_specs + 1; vt_num++) {
	 // printf("%lf\n", Qvalues[vt_num]);
  //}
  //for (int vt_num = 0; vt_num < num_layer_specs + 1; vt_num++) {
	 // printf("%lf\n", Rvalues[vt_num]);
