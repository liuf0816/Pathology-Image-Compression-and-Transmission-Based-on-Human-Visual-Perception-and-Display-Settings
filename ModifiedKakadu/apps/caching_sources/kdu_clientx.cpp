/*****************************************************************************/
// File: kdu_clientx.cpp [scope = APPS/CACHING_SOURCES]
// Version: Kakadu, V6.1
      ...
      if (num_comp_instructions == max_comp_instructions)
        { // Augment array
          ...
          kdcx_comp_instruction *tmp_inst =
            new kdcx_comp_instruction[max_comp_instructions];
  ...
  while (!top_level_complete)
    {
      ...
      if ((!top_box) || !top_box.is_complete())
        break; // Have to come back and continue parsing later
      if (top_box.get_box_type() == jp2_file_type_4cc)
        {
