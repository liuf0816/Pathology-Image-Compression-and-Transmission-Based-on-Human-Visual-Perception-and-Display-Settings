/*****************************************************************************/
// File: client_server_comms.cpp [scope = APPS/CLIENT-SERVER]
// Version: Kakadu, V6.1
  ...
  kd_tcp_channel::wait_event()
{
  if (timeout_milliseconds <= 0)
    return (WSAWaitForMultipleEvents(1,&event,FALSE,INFINITE,
                                     FALSE) == WSA_WAIT_EVENT_0);
  clock_t elapsed = clock();
  if (WSAWaitForMultipleEvents(1,&event,FALSE,(unsigned) timeout_milliseconds,
                               FALSE) != WSA_WAIT_EVENT_0)
    {
  ...
  timeout_milliseconds -= (int)(((double) elapsed)*(1000.0/CLOCKS_PER_SEC));
  if (timeout_milliseconds <= 0)
    timeout_milliseconds = 1;
  return true;
}
                       ...
                       bool *closure_request_flag)
{
  ...
  if (h_event == NULL)
    {
      if (internal_event == NULL)
        internal_event = CreateEvent(NULL,FALSE,FALSE,NULL); // Auto-reset
  ...
  unsigned long uparam = 1;
  ioctlsocket(sock,FIONBIO,&uparam); // Make socket non-blocking
  connect(sock,(SOCKADDR *) &address,sizeof(address));
  if (WSAEventSelect(sock,event,FD_CONNECT) != 0)
    {
      close();
      return false;
    }
  ...
  do {
      ...
      WSAEnumNetworkEvents(sock,NULL,&network_events);
    } while (network_events.lNetworkEvents == 0);
  if (network_events.iErrorCode[FD_CONNECT_BIT] ||
      !(network_events.lNetworkEvents & FD_CONNECT))
    {
      close();
      return false;
    }
  WSAEventSelect(sock,NULL,0); // Cancel event selection.
  event_flags = 0;
  timeout_milliseconds = 0;
  return true;
}
  ...
  kd_tcp_channel::close()
{
  if (sock != INVALID_SOCKET)
    {
      WSAEventSelect(sock,NULL,0); // Cancel any event selection.
  ...
  kd_tcp_channel::get_peer_address(SOCKADDR_IN &address)
{
  ...
  if (sock != INVALID_SOCKET)
    ...
    address.sin_addr.S_un.S_addr = INADDR_NONE;
}
  ...
  while (num_bytes > 0)
    {
      ...
      if (xfer_bytes == 0)
        ...
        { // Socket error or socket would block
          ...
          if (!blocking)
            {
              event_flags |= (FD_WRITE|FD_CLOSE);
              WSAEventSelect(sock,event,event_flags);
              return false; // Caller will have to come back later.
            }
          WSAEventSelect(sock,event,FD_WRITE|FD_CLOSE);
          bool wait_succeeded;
          if ((!(wait_succeeded=wait_event())) || *closure_request_flag)
            { // Caller will figure out what has gone wrong.
              if (suppress_errors || *closure_request_flag)
