...
void
  kdu_cache::add_to_databin(int cls, kdu_long codestream_id, kdu_long id,
                            kdu_byte data[], int offset, int num_bytes,
                            ...
                            bool mark_if_augmented)
{
  ...
  if ((stream == NULL) || (stream->codestream_id > codestream_id))
    { // Create new stream and insert into the list
      acquire_lock();
      kd_stream_cache *new_stream = new kd_stream_cache(codestream_id);
  ...
  for (seg=cache->root; seg->shift > 0; )
    {
      ...
      if (seg->segs[idx] == NULL)
        {
          seg->segs[idx] = new kd_var_cache_seg;
  ...
  if (offset <= head->initial_bytes)
    {
      num_bytes = offset+num_bytes-head->initial_bytes; // Amount left in range
      if (num_bytes < 0)
        num_bytes = 0;
      offset = 0;
    }
  ...
  bool existing_complete = (head->hole_list == NULL);
  int existing_offset=0, existing_bytes=0;
  bool augmented = false;

  // Merge until the new region is entirely accounted for.
  int merged_offset = offset;
  int merged_bytes = num_bytes;
  while (num_bytes > 0)
    {
      if ((existing_bytes == 0) && !existing_complete)
        { // Need to load a new source range
          existing_offset = hole_src.read_length();
          if (existing_offset == 0)
            existing_complete = true;
          else
            {
              existing_bytes = hole_src.read_length();
              assert(existing_bytes > 0);
            }
        }

      if ((existing_bytes == 0) || (existing_offset > (offset+num_bytes)))
        { // No more existing byte ranges intersect with new range
          if (num_bytes > 0)
            augmented = true;
          break;
        }
      ...
      if ((existing_offset < offset) &&
          ((existing_offset+existing_bytes) < offset))
        { // Existing range entirely precedes new range
          hole_dst.write_length(existing_offset);
          hole_dst.write_length(existing_bytes);
          assert(offset == merged_offset);
          offset -= (existing_offset+existing_bytes);
          merged_offset = offset;
          existing_offset = existing_bytes = 0;
          continue;
        }

      if (existing_offset <= offset)
        { // Adjust start position of the merged region
           assert(merged_offset == offset);
           num_bytes += offset-existing_offset;
           merged_offset = offset = existing_offset;
           merged_bytes = num_bytes;
        }
      else
        augmented = true;

      num_bytes -= (existing_offset+existing_bytes) - offset;
      if (num_bytes < 0)
        { merged_bytes -= num_bytes; num_bytes = 0; }
      existing_offset = existing_bytes = offset = 0;
    }

  if (merged_bytes > 0)
    { // Write the merged byte range
      if (merged_offset == 0)
        head->initial_bytes += merged_bytes;
      else
        {
          hole_dst.write_length(merged_offset);
          hole_dst.write_length(merged_bytes);
        }
    }

  // Copy any original ranges which have not yet been merged.
  while ((existing_bytes > 0) || !existing_complete)
    {
      if (existing_bytes == 0)
        { // Need to load a new source range
          existing_offset = hole_src.read_length();
          if (existing_offset == 0)
            { existing_complete = true; break; }
          existing_bytes = hole_src.read_length();
          assert(existing_bytes > 0);
        }
      hole_dst.write_length(existing_offset);
