...
static void
  print_banner(char *java_name, ostream &out)
{
  int i;
  ...
  implement_callback_binding(kdd_func *func, ostream &out)
{
  ...
  for (arg=func->args; nargs > 0; nargs--, arg=arg->next)
    out << ",_" << arg->name;
  out << ");\n";
  out << "    if (__env->ExceptionOccurred() != NULL) throw (int) 0;\n";
                              ...
                              const char *aux_dir)
{
  ...
  out << "\n";
  out << "static void checkForJavaException(JNIEnv *__env, bool badAlloc)\n";
  out << "{\n";
  out << "  if (__env->ExceptionOccurred() == NULL)\n";
  out << "    {\n";
  out << "        char *exception_class_name;\n";
  out << "        if (badAlloc)\n";
  ...
  out << "            return;\n";
  out << "        __env->ThrowNew(exception_class, \"Unexpected exception in Kdu library\");\n";
  out << "    }\n";
  out << "}\n";
  out << "\n";
            ...
            if (func->is_constructor && (func->java_name != NULL))
              {
                ...
                out << "    return ((jlong) _addr_to_kdu_long(self)) | "
                       "((jlong) 1);\n";
                out << "  } catch(std::bad_alloc) {checkForJavaException(__env,true);}\n";
                out << "  catch(...) {};\n";
                out << "  return (jlong) 0;\n";
                out << "}\n";
      ...
      for (func=cls->functions; func != NULL; func=func->next)
        if ((func->java_name != NULL) &&
            ...
            !(func->is_constructor || func->is_destructor))
          {
            ...
            out << "  } catch(std::bad_alloc) {checkForJavaException(__env,true);}\n";
            out << "  catch(...) {checkForJavaException(__env,false);}\n";
