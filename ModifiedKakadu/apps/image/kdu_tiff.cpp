...
void
  kdu_tiffdir::init(bool littlendian)
{
  close();
  ...
  kdu_tiffdir::opendir(kdu_compressed_source *in)
{
  ...
  tiff_parse(offset,head+4,littlendian);
  if (!in->seek(global_offset + ((kdu_long) offset)))
    return false;

  kdu_byte buf[12];

  kdu_uint16 entry_count;
  if ((in->read(buf,2) < 2) || (tiff_parse(entry_count,buf,littlendian)==0))
  ...
  for (; entry_count > 0; entry_count--)
    {
      ...
      kdu_uint32 tag_length, tag_offset;
      tiff_parse(tag_length,buf+4,littlendian);
      tiff_parse(tag_offset,buf+8,littlendian);
      ...
      if (elt->num_bytes <= 4)
        ...
        elt->location = global_offset + ((kdu_long) tag_offset);
      elt->next = tags;
      tags = elt;
    }
  ...
  kdu_tiffdir::get_dirlength()
{
  if (!is_open)
    return 0;
  kdu_uint32 result=2+4; // Entry count (2) + null terminator (4)
  kd_tifftag *scan;
  for (scan=tags; scan != NULL; scan=scan->next)
    {
      kdu_uint32 num_bytes = scan->num_bytes;
      if (num_bytes == 0)
  ...
  kdu_tiffdir::writedir(kdu_compressed_target *tgt, kdu_long dir_offset)
{
  ...
  kdu_uint16 entry_count=0;
  kdu_long expected_dirlength = 6;
  for (scan=tags; scan != NULL; scan=scan->next)
    if (scan->num_fields > 0)
      {
        entry_count++;
        expected_dirlength += 12;
      }
  ...
  kdu_long out_length = 2; // Includes 2-byte count above
  for (scan=tags; scan != NULL; scan=scan->next)
    {
      if (scan->num_fields == 0)
        continue;
      ...
      if (scan->num_bytes > 4)
        { // Generate pointer to the actual data
          ...
          extra_offset += (kdu_long) scan->num_bytes;
        }
      ...
      { // Need to write extra data
        ...
        if (scan->out_buf != NULL)
          ...
          { // We must read contents first
            assert(src != NULL);
            kdu_byte buf[256]; // Temporary buffer for read/write exchange
            kdu_uint32 xfer_bytes, bytes_left=scan->num_bytes;
            kdu_long pos = scan->location;
            for (; bytes_left > 0; bytes_left-=xfer_bytes, pos+=xfer_bytes)
              {
                xfer_bytes = (bytes_left>256)?256:bytes_left;
                read_bytes(buf,(int) xfer_bytes,pos,scan->tag_type);
...
void
  kdu_tiffdir::read_bytes(kdu_byte *buf, int num_bytes,
                          kdu_long pos, kdu_uint32 tag_type)
{
  ...
  if (src->read(buf,num_bytes) < num_bytes)
...
int
  kdu_tiffdir::read_tag(kdu_uint32 tag_type, int length, kdu_byte data[])
{
  assert(length >= 0);
  kd_tifftag *elt = find_tag(tag_type);
  if (elt == NULL)
    return 0;
  kdu_uint32 remaining_bytes = elt->num_bytes-elt->read_ptr;
  if (remaining_bytes < (kdu_uint32) length)
    length = (int) remaining_bytes;
  if (length == 0)
    return 0;
  if (elt->out_buf != NULL)
    memcpy(data,elt->out_buf+elt->read_ptr,(size_t) length);
  else if (elt->num_bytes <= 4)
    memcpy(data,elt->data+elt->read_ptr,(size_t) length);
  else
    read_bytes(data,length,elt->location+(kdu_long)(elt->read_ptr),
               elt->tag_type);
  elt->read_ptr += (kdu_uint32) length;
  return length;
}
...
int
  kdu_tiffdir::read_tag(kdu_uint32 tag_type, int length, kdu_uint16 data[])
{
  assert(length >= 0);
  ...
  kdu_uint32 remaining_fields = (elt->num_bytes-elt->read_ptr)>>1;
  if (remaining_fields < (kdu_uint32) length)
    length = (int) remaining_fields;
  if (length == 0)
    return 0;
  int len_bytes = (length<<1);
  if (elt->out_buf != NULL)
    memcpy(data,elt->out_buf+elt->read_ptr,(size_t) len_bytes);
  else if (elt->num_bytes <= 4)
    memcpy(data,elt->data+elt->read_ptr,(size_t) len_bytes);
  else
    read_bytes((kdu_byte *) data,len_bytes,
               elt->location+(kdu_long)(elt->read_ptr),elt->tag_type);
  elt->read_ptr += (kdu_uint32) len_bytes;
...
int
  kdu_tiffdir::read_tag(kdu_uint32 tag_type, int length, kdu_uint32 data[])
{
  assert(length >= 0);
  kdu_uint16 data_type = (kdu_uint16)(tag_type);
  if ((data_type == 3) || (data_type == 8))
    { // Read as 16-bit data and then upconvert
      int n;
      length = read_tag(tag_type,length,(kdu_uint16 *) data);
      if (data_type == 3)
        { // Unsigned data
          kdu_uint16 *sp=(kdu_uint16 *) data;
          kdu_uint32 *dp=(kdu_uint32 *) data;
          for (n=length-1; n >= 0; n--)
            dp[n] = sp[n];
        }
      else
        { // Signed data
          kdu_int16 *sp=(kdu_int16 *) data;
  ...
  if ((elt->read_ptr & 3) != 0)
    { kdu_error e; e << "Misaligned access to structured data fields in a "
      ...
      "calls to the unstructured byte-wise `read_tag' functions with one "
      "of the structured value-wise `read_tag' functions."; }
  kdu_uint32 remaining_fields = (elt->num_bytes-elt->read_ptr)>>2;
  if (remaining_fields < (kdu_uint32) length)
    length = (int) remaining_fields;
  if (length == 0)
    return 0;
...
int
  kdu_tiffdir::read_tag(kdu_uint32 tag_type, int length, double data[])
{
  assert(length >= 0);
  kdu_uint16 data_type = (kdu_uint16)(tag_type);
  ...
  if ((elt->read_ptr % elt->bytes_per_field) != 0)
    { kdu_error e; e << "Misaligned access to structured data fields in a "
      ...
      "calls to the unstructured byte-wise `read_tag' functions with one "
      "of the structured value-wise `read_tag' functions."; }
  kdu_uint32 remaining_fields =
    (elt->num_bytes-elt->read_ptr) / elt->bytes_per_field;
  if (remaining_fields < (kdu_uint32) length)
    length = (int) remaining_fields;
  if (length == 0)
    return 0;
  int len_bytes = (length * (int) elt->bytes_per_field);
  if (elt->out_buf != NULL)
    memcpy(data,elt->out_buf+elt->read_ptr,(size_t) len_bytes);
  else if (elt->num_bytes <= 4)
    memcpy(data,elt->data+elt->read_ptr,(size_t) len_bytes);
  ...
  if (native_littlendian != littlendian)
    {
      ...
      int num_dwords=(len_bytes>>2);
      kdu_uint32 val, *wp = (kdu_uint32 *) data;
  ...
  if (data_type == 12)
    ...
    { // Convert from unsigned rational
      kdu_uint32 *sp=(kdu_uint32 *) data;
      for (n=0; n < length; n++, sp+=2)
        data[n] = (sp[1]==0)?0.0:(((double) sp[0]) / ((double) sp[1]));
    }
  else if (data_type == 10)
    { // Convert from signed rational
      kdu_int32 *sp=(kdu_int32 *) data;
      for (n=0; n < length; n++, sp+=2)
        data[n] = (sp[1]==0)?0.0:(((double) sp[0]) / ((double) sp[1]));
    }
  else
  ...
  kdu_tiffdir::write_tag(kdu_uint32 tag_type, int length, kdu_byte data[])
{
  ...
  assert(elt != NULL);
  kdu_uint32 new_total_bytes = elt->num_bytes + (kdu_uint32) length;
  ...
  memcpy(elt->out_buf+elt->num_bytes,data,(size_t) length);
  ...
  kdu_tiffdir::write_tag(kdu_uint32 tag_type, int length, kdu_uint16 data[])
{
  ...
  kdu_uint32 new_total_bytes = elt->num_bytes + (kdu_uint32)(length<<1);
  if (new_total_bytes > elt->max_bytes)
    { // Allocate or reallocate buffer
      elt->max_bytes += 4 + new_total_bytes;
      kdu_byte *new_buf = new kdu_byte[elt->max_bytes];
      if (elt->out_buf != NULL)
        {
          memcpy(new_buf,elt->out_buf,(size_t)(elt->num_bytes));
  ...
  kdu_uint16 val, *dp = (kdu_uint16 *)(elt->out_buf+elt->num_bytes);
  memcpy(dp,data,(size_t)(new_total_bytes-elt->num_bytes));
  if (native_littlendian != littlendian)
  ...
  kdu_tiffdir::write_tag(kdu_uint32 tag_type, int length, kdu_uint32 data[])
{
  ...
  if ((elt->num_bytes & 3) != 0)
    { kdu_error e; e << "Misaligned access to structured data fields in a "
      ...
      "calls to the unstructured byte-wise `write_tag' functions with one "
      "of the structured value-wise `write_tag' functions."; }
  kdu_uint32 new_total_bytes = elt->num_bytes + (kdu_uint32)(length<<2);
  if (new_total_bytes > elt->max_bytes)
    { // Allocate or reallocate buffer
      elt->max_bytes += 4 + new_total_bytes;
      kdu_byte *new_buf = new kdu_byte[elt->max_bytes];
  ...
  kdu_uint32 val, *dp = (kdu_uint32 *)(elt->out_buf+elt->num_bytes);
  memcpy(dp,data,(size_t)(new_total_bytes-elt->num_bytes));
  if (native_littlendian != littlendian)
    { // Flip bytes in each word
      for (int n=0; n < length; n++)
  ...
  kdu_tiffdir::write_tag(kdu_uint32 tag_type, int length, double data[])
{
  ...
  if (elt->num_bytes != (elt->num_fields*elt->bytes_per_field))
    { kdu_error e; e << "Misaligned access to structured data fields in a "
      ...
      "calls to the unstructured byte-wise `write_tag' functions with one "
      "of the structured value-wise `write_tag' functions."; }
  kdu_uint32 new_total_bytes =
    elt->num_bytes + ((kdu_uint32) length)*elt->bytes_per_field;
  if (new_total_bytes > elt->max_bytes)
    { // Allocate or reallocate buffer
      elt->max_bytes += 4 + new_total_bytes;
      kdu_byte *new_buf = new kdu_byte[elt->max_bytes];
  ...
  if (data_type == 12)
    { // Double precision
      double *dp = (double *)(elt->out_buf+elt->num_bytes);
      for (n=0; n < length; n++)
        dp[n] = data[n];
    }
  else if (data_type == 11)
    { // Upconvert to single precision floats
      float *dp=(float *)(elt->out_buf+elt->num_bytes);
      for (n=0; n < length; n++)
        dp[n] = (float) data[n];
    }
  else if (data_type == 5)
    { // Convert to unsigned rational
      kdu_uint32 *dp=(kdu_uint32 *)(elt->out_buf+elt->num_bytes);
    ...
    { // Convert to signed rational
      kdu_int32 *dp=(kdu_int32 *)(elt->out_buf+elt->num_bytes);
      for (n=0; n < length; n++, dp+=2)
        {
          double val = data[n];
          if (val < 0.0)
  ...
  if (littlendian != native_littlendian)
    {
      // Start by flipping bytes inside each 32-bit word
      assert(elt->bytes_per_field >= 4);
      int num_dwords = ((new_total_bytes-elt->num_bytes)>>2);
      kdu_uint32 val, *wp = (kdu_uint32 *)(elt->out_buf+elt->num_bytes);
      for (n=0; n < num_dwords; n++)
        {
          val = wp[n];
          val = (val>>24) + ((val>>8) & 0x0000FF00)
  ...
  kdu_tiffdir::copy_tag(kdu_tiffdir &src, kdu_uint32 tag_type)
{
  kdu_byte buf[256];
  int xfer_bytes;
  while ((xfer_bytes = src.read_tag(tag_type,256,buf)) > 0)
    {
