...
class kdu_image_dims {
  ...
  */
  public: // Member functions
    kdu_image_dims()
      {
        ...
        xpels_per_metre = ypels_per_metre = 0.0;
    ...
    void set_resolution(int ref_width, int ref_height, bool units_known,
    ...
    bool get_resolution(int comp_idx, bool &units_known,
      ...
      { /* [SYNOPSIS]
        ...
        */
        ...
        double scale_x = get_width(comp_idx) / (double) ref_width;
        double scale_y = get_height(comp_idx) / (double) ref_height;
    ...
    void append_component()
      ...
      */
        ...
        if (num_components == 0)
          ...
          {
            data[num_components*3+0] = data[num_components*3-3];
            data[num_components*3+1] = data[num_components*3-2];
            data[num_components*3+2] = data[num_components*3-1];
          }
        num_components++;
      }
    void add_component(int height, int width, int bit_depth, bool is_signed,
      ...
      */
        ...
        while (num_components <= comp_idx)
          append_component();
        data[comp_idx*3+0] = height;  data[comp_idx*3+1] = width;
        data[comp_idx*3+2] = (is_signed)?(-bit_depth):bit_depth;
      }
    ...
    void set_forced_precision(int comp_idx, int precision)
      { int c;
        ...
        forced_precisions[comp_idx] = precision;
      }
      /* [SYNOPSIS]
    ...
    int get_forced_precision(int comp_idx)
      ...
      /* [SYNOPSIS]
           ...
           (via `set_bit_depth') to the forced precision.  If the original
           data precision associated with the data samples recorded in
           the file is larger than the forced precision, the samples with
           forced precision should be obtained by clipping, rather than
           just ignoring the more significant bits.
           [//]
           If an image fiel writer supports precision forcing, it should
           interpret a non-zero value returned by this function as the
           ...
           `kdu_image_out::put' function.  If that value is larger than
           the forced precision, the values to be written to the file should
           be obtained by clipping, rather than just ignoring the more
           significant bits.
      */
  ...
  private: // Private data
    ...
    int ref_width, ref_height; // Reference dimensions for resolution info
