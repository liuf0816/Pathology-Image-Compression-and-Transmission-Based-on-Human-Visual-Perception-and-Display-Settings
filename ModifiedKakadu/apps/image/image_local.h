...
class tif_in : public kdu_image_in_base {
  public: // Member functions
    ...
    bool get(int comp_idx, kdu_line_buf &line, int x_tnum);
  private: // Helper functions
    void perform_buffer_unpack(kdu_byte *src, kdu_byte *dst);
      /* Works on a single chunk line */
    void perform_sample_remap(kdu_byte *buf);
  ...
  private: // Members describing the organization of the TIFF data
    ...
    int chunks_across; // Number of chunks across image width
    ...
    kdu_uint32 *chunk_offsets; // NULL if `libtiff_in' used -- see below.
