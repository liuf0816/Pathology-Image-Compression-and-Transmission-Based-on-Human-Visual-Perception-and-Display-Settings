...
-(bool)initWithCodestream:(kdu_codestream)cs
{
  ...
  try {
    ...
    const char *comtext;
    while ((comtext = (com = cs.get_comment(com)).get_text()) != NULL)
      {
  ...
  catch (int) { // Error occurred while accessing codestream
    return false;
  }
         ...
         atRow:(NSInteger)row column:(NSInteger)column
{
  ...
  NSString *mod_text =
    [ns_text stringByReplacingOccurrencesOfString:@"\n" withString:delim];
  [cell setStringValue:mod_text];
