...
/*****************************************************************************/
/* STATIC INLINE                 make_unichar                                */
/*****************************************************************************/

static inline unichar make_unichar(char ch)
  ...
  if (!child_list_complete)
    { // Add onto the child list, if we can
      ...
      if (parent == NULL)
        ...
        { // Open child sub-boxesin sequence, until we get to the end of the
...
- (void) update_metadata
{
  ...
  bool any_change = tree->update_structure(src,meta_manager);
  if (any_change)
    {
...
- (void) update_info_text
{
  ...
  if (display_node->box_type == jp2_file_type_4cc)
    ...
    { // Append plain text contents of box
      if (display_node_content_bytes == 0)
        [info_text_frame setTitle:
      ...
      while (max_chars_left>=0)
        {
          ...
          for (int n=0; n < num_chars; n++)
            unibuf[n] = make_unichar(buf[n]);
          NSString *text = [NSString stringWithCharacters:unibuf
                                                   length:num_chars];
...
/*****************************************************************************/
/*                 kdms_metashow:initWithOwner:andIdentifier                 */
/*****************************************************************************/
        ...
        andIdentifier:(int)the_identifier
{
  ...
  // Create the window
  NSRect frame_rect = [[NSScreen mainScreen] frame];
  ...
  [text_panel_frame setFillColor:[NSColor grayColor]];
...
- (void) select_matching_metanode:(jpx_metanode)jpx_node
{
  [self update_metadata]; // Best to do this first, to maximize our chance of
